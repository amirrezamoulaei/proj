    ...
    : currentUser(nullptr)
{
    ...
    users[adminUser] = new User(adminUser,
                                adminPassHash,
                                "System Admin",
                                "09120000000",
                                "admin@kalanet.com",
...
void KalaNetCore::signup(std::string username,
                         std::string password,
                         std::string name,
                         std::string phone,
                         std::string email)
...
void KalaNetCore::approveAd(std::string adId)
{
    ...
    if (ads.find(adId) == ads.end())
        throw AdException("Ad ID not found.");
...
{
    if (!currentUser)
        throw AuthException("Login required to shop.");

    std::lock_guard<std::mutex> lock(dataMutex);

    auto it = ads.find(adId);
    if (it == ads.end())
        throw AdException("Ad not found.");
...
void KalaNetCore::checkout()
{
    ...
    std::vector<Ad *> items = getCartItems();
    if (items.empty())
        throw AdException("Cart is empty.");
...
std::vector<Transaction> KalaNetCore::getUserHistory()
{
    ...
    for (const auto &t : transactions) {
        if (t.buyerUser == currentUser->getUsername()
            || t.sellerUser == currentUser->getUsername()) {
            result.push_back(t);
        }
    }
    return result;
